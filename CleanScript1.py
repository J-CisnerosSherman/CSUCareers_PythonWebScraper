...
def extract_c_infoo(doc,pattern_number):
    #creates a matcher object
    matcher = Matcher(nlp.vocab)
    #Selects patterns based on the number
    patterns =select_patterns(pattern_number)
    ...
    matches = matcher(doc)
    print ("matches ", matches)
    print ("it has this many matches ", len(matches))
    ...
    for match_id, start, end in matches:
        st = start
        keyword = doc[start]   # Contact word
        print ("key word",keyword, "Start", start, "match_id", match_id)
        department_name = doc[start + 1:end]
        print ("dept nae here ", department_name)
    ...
    if pattern_num == 1 :
        ...
        for index_point in indexlist:
            ...
            if closest_entity is None:
                ...
                contact_i = extract_cc_infoo(doc,4) 
                print("what we got instead ", contact_i)
                closest_entity = contact_i["Contact"]
                print("what we got instead ", closest_entity)
                


            keyword = doc[index_point] #gets the keyword at index in this case contact 
            keyword_text = keyword.text
