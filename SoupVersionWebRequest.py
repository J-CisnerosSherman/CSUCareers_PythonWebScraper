...
def search_job_list():
    ...
    data = {
        ...
        'jobposted': '20',                      # it only works with 0,10,20 increment of 10 no other days work
...
def get_job_details(job):
    ...
    try:
        ...
        if response.status_code == 200:   #this is the url of each job listing dependig by campus ?
            ...
            # call function that passes the soup object to parse , and  an int to recognize campus
            # must return an dictionary, and then add it to the details dictionary
            # details.update(get_job_details2(soup, campus))
...
def get_description_details(soup, campus_num):
    ...
    if description_span:
                ...
                if description_strong:  #If it finds it then extract html content to parse it 
                    # function that calls to further parse the html content, takes an int as an argument to know which campus is 
                    #being processed
                    ...
                    #will call other functions from somewhere here 
