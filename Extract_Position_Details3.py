...
def dept_details(sublist1, sublist2, sublist3):
    ...
    if sublist1:
        ...
        doc = nlp(sublist_string)                                 #creates a doc object based on the new sublist_string
        ...
        for pattern_num in range(3):                              # Loop through the three different variables to find 
            ...
            matches = matcher(doc)                              # finds matches in the doc object

            if matches:                                         
                match_id, start, end = matches[0]               # get the first match of the corresponding pattern
                span = doc[start:end]                           # get the matched span (until the colon)
                newline_token = None
                ...
                text_after_span = doc[end:newline_token.i].text
...
def review_date(sublist1, sublist2, sublist3):
    ...
    patterns2 = [#Pattern 0  Corresponds to College: in output_dictionary 
            [{"LEMMA": {"IN": similar_words}},{"LOWER": "of"},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_words2}},
             {"IS_ALPHA": True, "OP": "*"},{"LOWER": "will", "OP":"?"}, {"LEMMA": {"IN": similar_words3}}],  #the ? means 0 or more times
            [{"LEMMA": {"IN": similar_words2}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_words5}},
             {"IS_ALPHA": True, "OP": "*"}, {"LEMMA": {"IN": similar_words4}}],
            [{"LEMMA": {"IN": similar_words6}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_words2}, "OP": "?"},
             {"LEMMA": {"IN": similar_words4}}],
            [{"LEMMA": {"IN": similar_words}},{"LEMMA": {"IN": similar_words3}}]            
        ]
    ...
    if sublist1:
        ...
        if matches:
            review_dates = []
            for match_id, start, end in matches:             # get the first match of the corresponding pattern
                ...
                for token in doc[end:]:
                    if token.text == '\n':
                        newline_token = token
                        break
                end_index = newline_token.i if newline_token else len(doc)
                span_doc = doc[end:end_index]
    ...
    if output_dict["ReviewDate"] is None:          #checks if any of the values are empty in the output dictionary
        ...
        if sublist2: 
            ...
            matches = matcher(doc2)                              # finds matches in the doc object
    ...
    if output_dict["ReviewDate"] is None:          #checks if any of the values are empty in the output dictionary
        if sublist3: 
            ...
            matches = matcher(doc3)                              # finds matches in the doc object
...
def contact_info(my_list):                      #Given a String splits it into 3 lists based on the number of colons and the number of words in the line
    ...
    patterns = [#Pattern 0  Corresponds to College: in output_dictionary 
            ...
            [{"LEMMA": {"IN": similar_wordsA}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsB}},
             {"IS_ALPHA": True, "OP": "*"},{"LOWER": "to"}],
            [{"LEMMA": {"IN": similar_wordsB}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsD}},
             {"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsE}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsF}}],  #the ? means 0 or more times
            [{"LEMMA": {"IN": similar_wordsB}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsC}},
             {"IS_ALPHA": True, "OP": "*"},{"LOWER": "to"}],
            [{"LOWER": "for"},{"LEMMA": {"IN": similar_wordsB}}, {"LOWER": "and"},{"LEMMA": {"IN": similar_wordsB}}]            
        ]

    pattern2 =[#Pattern 0  Corresponds to College: in output_dictionary 
            {"LEMMA": {"IN": similar_wordsG},"OP": "?"},{"IS_UPPER": True, "LENGTH": 1,"OP": "?"},{"TEXT": ".","OP": "?"},{"POS": "PROPN", "IS_TITLE": True},
    ...
    pattern3 = [ #Pattern to recognize phone numbers  
            ...
            {"SHAPE": "dddd"},{"IS_SPACE": True, "OP": "?"},{"ORTH": ".","OP":"?"},{"ORTH": "-","OP":"?"}
        ]
    ...
    if my_list:
        ...
        if matches:
            ...
            for doc_sub in sub_docs_to_check:  #for each doc that was formed from the new line tokens
                ...
                matcher = Matcher(nlp.vocab)
                matcher.add("Contact Name", [pattern2])
                matches = matcher(doc_sub)
                ...
                if matches:   
                    ...
                    if len(matches) == 1:               
                        ...
                        match_id, startB, endB = matches[1]
                ...
                matcher2.add("Phone Number", [pattern2])
                ...
                if matches2:
                    ...
                    if len(matches2) == 1:               
                        ...
                        match_id, startB, endB = matches2[1]
                        span2 = doc_sub[startB:endB]
                        textB = span2.text
