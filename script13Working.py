...
def extract_hiring_department_infoo(text):
    ...
    for match_id, start, end in matches:
        ...
        department_name = doc[start + 1:end]
        print ("dept nae here ", department_name)
        start_index = keyword.idx
        end_index = department_name[-1].idx + len(department_name[-1].text)
        
        if keyword.text not in department_info:
            department_info[keyword.text] = {"start_index": start_index, "end_index": end_index}
            print("Added to department_info:", keyword.text, "start_index:", start_index, "end_index:", end_index)
...
print("fjj ", dept)
#deadline_date = extract_deadline_date(c_text)
#print("Extracted Deadline Date:", deadline_date)
doc2 = nlp(c_text)
index = 21 
token_at_index = doc2[index]
print(f"Token at index {index}: {token_at_index.text}")
print(f"Token's lemma: {token_at_index.lemma_}")
print(f"Token's part-of-speech tag: {token_at_index.pos_}")

#index = 154 
#specific_word = c_text[index]
#print(f"Word at index {index}: {specific_word}")
