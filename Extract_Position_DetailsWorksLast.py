...
def review_date(sublist1, sublist2, sublist3):
    ...
    patterns2 = [#Pattern 0  Corresponds to College: in output_dictionary 
            [{"LEMMA": {"IN": [word.lower() for word in similar_words]}},{"LOWER": "of"},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_words2}},
             {"IS_ALPHA": True, "OP": "*"},{"LOWER": "will", "OP":"?"}, {"LEMMA": {"IN":[word.lower() for word in similar_words3]}}],  #the ? means 0 or more times
            [{"LEMMA": {"IN": [word.lower() for word in similar_words2]}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": [word.lower() for word in similar_words5]}},
             {"IS_ALPHA": True, "OP": "*"}, {"LEMMA": {"IN":[word.lower() for word in similar_words4]}}],
            [{"LEMMA": {"IN": [word.lower() for word in similar_words6]}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": [word.lower() for word in similar_words2]}, "OP": "?"},
             {"LEMMA": {"IN":[word.lower() for word in  similar_words4]}}],
            [{"LEMMA": {"IN": [word.lower() for word in similar_words]}},{"LEMMA": {"IN":[word.lower() for word in similar_words3]}}]            
        ]
    """
    patterns2 = [#Pattern 0  Corresponds to College: in output_dictionary 
    ...
    date_patterns = [
        ...
        [{"SHAPE": "dd/dd/dddd"}],[{"SHAPE": "d/d/dddd"}],[{"SHAPE": "dd/d/dd"}],[{"SHAPE": "d/dd/dd"}],
        [{"SHAPE": "dd-dd-dddd"}],[{"SHAPE": "d-d-dddd"}],[{"SHAPE": "dd-d-dd"}],[{"SHAPE": "d-dd-dd"}],
        [
         {"TEXT": {"regexp": r"\d{2}/\d{2}/\d{2},$"}},
        ],
    ...
    if sublist1:
        ...
        if matches1:
            ...
            for match_id, start, end in matches1B:             
                ...
                for token in doc_dateA:
                    print(f"Text: {token.text}, Lemma: {token.lemma_}, POS: {token.pos_}, Tag: {token.tag_}, is_alpha: {token.is_alpha}, is_digit: {token.is_digit}")
...
def contact_info(my_list):                      #Given a String splits it into 3 lists based on the number of colons and the number of words in the line
    ...
    patterns = [#Pattern 0  Corresponds to College: in output_dictionary 
            ...
            [{"LEMMA": {"IN": [word.lower() for word in similar_wordsA]}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": [word.lower() for word in similar_wordsB]}},
             {"IS_ALPHA": True, "OP": "*"},{"LOWER": "to"}],
            [{"LEMMA": {"IN": [word.lower() for word in similar_wordsB]}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": [word.lower() for word in similar_wordsD]}},
             {"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsE}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": similar_wordsF}}],  #the ? means 0 or more times
            [{"LEMMA": {"IN": [word.lower() for word in  similar_wordsB]}},{"IS_ALPHA": True, "OP": "*"},{"LEMMA": {"IN": [word.lower() for word in similar_wordsC]}},
             {"IS_ALPHA": True, "OP": "*"},{"LOWER": "to"}],
            [{"LOWER": "for"},{"LEMMA": {"IN": [word.lower() for word in similar_wordsB]}}, {"LOWER": "and"},{"LEMMA": {"IN": [word.lower() for word in similar_wordsB]}}]            
        ]
    """
