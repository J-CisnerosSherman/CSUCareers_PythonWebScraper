...
def get_Keywords(element):
            ...
            if text != "" :
                ...
                if num_words <= 15:
                    ...
                    if pair not in word_key:
                        print("new key word found", text, "at tag", tag.name)
                        word_key.add(pair)
    ...
    #key_tags = element.find_all(['h3', 'strong', 'em', 'u', 'span'], style='text-decoration: underline')
    
    #key_word_list = [tag.get_text() for tag in key_tags]
    
    #print("Strong text list",key_word_list)
...
def clean_htmlchars(element):
    
    div_tags = element.find_all('div')
    if div_tags:
        for div_tag in div_tags:                        #using this one actually prints the string twice
            if div_tag.string:                          #can maybe replace withits contents. 
    ...
    for tag in element.find_all(lambda tag: '\u2010' in tag.text):
...
#This one removes all non ascii chars and cleans the text and replaces it in the tagstring changing descrition span forevaa
def remove_nbsp2(element):
    #clean_description_text = ""
    for tag in element.find_all():
        print("it got here")
        ...
        if tag.string is not None and '\xa0' in tag.string and '\u2010' in tag.string:
            print("got them both")
            tag_text = tag.text.strip()
            cleaned_text = tag_text.replace('\xa0', ' ').replace('\u2010', ' ')

            clean_text = unicodedata.normalize('NFKD', cleaned_text)
            clean_text = clean_text.encode('ASCII', 'ignore').decode('utf-8')
            tag.string.replace_with(clean_text)
            print("replace both espcial chars at ", tag.name)
            #clean_description_text += clean_text + "\n"

        elif tag.string is not None and '\xa0' in tag.string:
            print("got them")
            tag_text = tag.text.strip()
            cleaned_text = tag_text.replace('\xa0', ' ')
        ...
        elif tag.string is not None and '\u2010' in tag.string:
            print("got them 2")
            tag_text = tag.text.strip()
            cleaned_text = tag_text.replace('\u2010', ' ')
            ...
            clean_text = clean_text.encode('ASCII', 'ignore').decode('utf-8')
            tag.string.replace_with(clean_text)
            print("replaced the dash at", tag.name)
            #clean_description_text += clean_text + "\n"
            ...
            clean_text = unicodedata.normalize('NFKD', tag_text)
            clean_text = clean_text.encode('ASCII', 'ignore').decode('utf-8')
            tag.string.replace_with(clean_text)
            print("no especial chars at", tag.name)  
            #clean_description_text += clean_text + "\n"  
...
def remove_dash(element):
    ...
    for tag in element.find_all():
        ...
        if tag.string is not None and '\xa0' in tag.string: 
            ...
            print("tag text", tag_text)
            cleaned_text = tag_text.replace('\xa0', ' ')
            clean_text = unicodedata.normalize('NFKD', cleaned_text).encode('ASCII', 'ignore').decode('utf-8')
            print("cleaned text", cleaned_text)
...
def get_other_kwords(element):
    ...
    for tag in element.find_all():
        ...
        if substring not in word_key and substring != "":
            word_list = substring.split()
            num_words = len(word_list)
            if num_words < 15:
                print("new OTHER word found", substring)
                word_key.add(substring)
                print("adding other key words" , substring)
...
for campus_num, campus_name in campus_dict.items():
    ...
    if results:
        ...
        print("Keywords for campus +++++++++++", campus_name , "++++++++++\n\n" ,word_key)
        word_key.clear()
...
if results:
    ...
    print ("global key words by campus", word_key)
else:
    print("No Job listings found.")
