...
def search_job_list():
    ...
    data = {
        ...
        'campusList': ['14'],                   # ['1'] (See list in comment block above), None (All)                   
        ...
        if response.status_code == 200:   #this is the url of each job listing dependig by campus ?
            ...
            if description_span:
                ...
                for child in children_tags:
                    ...
                    #You can use any string methods 
                    #you can maybe set some of the pattern findings here 
...
def has_text2(tag):
    return tag.string and not isinstance(tag.string, Comment) and tag.name != 'title'

# find all tags that contain text using the filter function
tags_with_text = soup.find_all(has_text)

# extract the text from each tag and print it
